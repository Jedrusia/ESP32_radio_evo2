...
int buttonLongPressTime1 = 1000;  // Czas reakcji na długie nacisniecie enkoder 1
int buttonLongPressTime2 = 1000; // Czas reakcji na długie nacisniecie enkoder 2
...
//U8G2_SH1122_256X64_F_4W_HW_SPI u8g2(U8G2_R0, /* cs=*/ 42, /* dc=*/ 40, /* reset=*/ 41);				// Enable U8G2_16BIT in u8g2.h
 ...
 // static unsigned long lastPressTime = 0;    // Zmienna do kontrolowania debouncingu (ostatni czas naciśnięcia)
  ...
  // Debouncing dla przycisku enkodera 2
    ...
    if (reading2 == LOW)  // Przycisk jest wciśnięty (stan niski)
    {
      ...
      // Jeśli przycisk jest wciśnięty przez co najmniej 3 sekundy i akcja jeszcze nie była wykonana
      if (millis() - buttonPressTime2 >= buttonLongPressTime2 && !action2Taken)
      {
...
{ 
  ...
  if (stationsCount < MAX_STATIONS)
  {
    ...
    if (length <= STATION_NAME_LENGTH)
    {
      ...
      u8g2.drawStr(10,36, "Pobrano stacji:");
      u8g2.drawStr(100,36, String(stationsCount).c_str()); // Napisz licznik pobranych stacji
...
void fetchStationsFromServer()
{
  ...
  {  
    ...
    u8g2.drawStr(130, 25, "serwera GitHub");			 
    {
...
void displayRadio()
{
  ...
  int yPosition = 28;  // Początkowa pozycja Y
...
void updateTimer()
{
  ...
  if (timeDisplay == true)
  {
    if (audio.isRunning() == true)
    {
      if (mp3 == true)
      {
        u8g2.drawStr(170, 63, " MP3");
        //Serial.println("Gram MP3");
      }
      if (flac == true)
      {
        u8g2.drawStr(170, 63, "FLAC");
        //Serial.println("Gram FLAC");
      }
      if (aac == true)
      {
        u8g2.drawStr(170, 63, " AAC");
...
*/
    ...
    else  // Regulacja głośności
    {
      ...
      u8g2.clearBuffer();
      u8g2.setFont(u8g2_font_fub14_tf);
      u8g2.drawStr(90, 33, "VOL:");
      u8g2.drawStr(142, 33, volumeValueStr.c_str());
